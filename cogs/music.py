...
# ── Auto-install PyNaCl if missing (fixes "nacl/davey library needed") ────────
try:
    import nacl  # noqa: F401
except ImportError:
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "PyNaCl>=1.5.0"],
...
class Music(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        if not _check_nacl():
            print("[Music] WARNING: PyNaCl not found - voice will not work.")
        else:
            print("[Music] PyNaCl found - voice support ready.")
    ...
    async def safe_connect(self, interaction: discord.Interaction, state: GuildMusicState) -> bool:
        if state.voice_client and state.voice_client.is_connected():
            return True
        if not _check_nacl():
            await interaction.followup.send(
                "**Voice Error: PyNaCl / libsodium not found.**\n\n"
                "**Railway fix:** Make sure your `nixpacks.toml` has:\n"
                "```toml\n[phases.setup]\nnixPkgs = [\"python311\", \"ffmpeg\", \"libsodium\"]\n```\n"
                "Push that file to GitHub and redeploy on Railway."
            )
            return False
        try:
            ...
            await interaction.followup.send("Timed out connecting to voice. Please try again.")
            return False
        except Exception as e:
            err = str(e).lower()
            if any(x in err for x in ["nacl", "sodium", "opus", "davey"]):
                await interaction.followup.send(
                    "**Voice Error: PyNaCl / libsodium not found.**\n\n"
                    "**Railway fix:** Add `libsodium` to `nixpacks.toml` nixPkgs and redeploy.\n"
                    "**Local fix:** `pip install PyNaCl>=1.5.0` then restart the bot."
                )
            else:
    ...
    async def fetch_song(self, query: str, requester: discord.Member) -> Song | None:
        ...
        try:
            data = await loop.run_in_executor(None, lambda: yt_dlp.YoutubeDL(ydl_opts).extract_info(query, download=False))
            if not data:
                return None
            if "entries" in data:
                ...
                if not entry.get("url"):
                    entry = await loop.run_in_executor(None, lambda: yt_dlp.YoutubeDL(ydl_opts).extract_info(entry.get("webpage_url") or "", download=False))
            else:
                entry = data
    ...
    async def fetch_playlist(self, url: str, requester: discord.Member) -> list[Song]:
        ...
        try:
            ...
            for entry in data["entries"]:
                if entry:
                    songs.append(Song({"title": entry.get("title", "Unknown"), "url": entry.get("url") or f"https://www.youtube.com/watch?v={entry.get('id', '')}", "webpage_url": f"https://www.youtube.com/watch?v={entry.get('id', '')}", "thumbnail": entry.get("thumbnail", ""), "duration": entry.get("duration", 0), "channel": entry.get("channel") or entry.get("uploader", "Unknown")}, requester))
            return songs
        except Exception as e:
    ...
    async def play_next(self, guild_id: int):
        ...
        if state.text_channel:
            try:
                await state.text_channel.send(embed=self._now_playing_embed(song))
            except Exception:
                pass

    def _now_playing_embed(self, song: Song) -> discord.Embed:
        embed = discord.Embed(title="Now Playing", description=f"[{song.title}]({song.webpage_url})", color=discord.Color.purple(), timestamp=datetime.datetime.utcnow())
    ...
    @app_commands.command(name="play", description="Play a song — YouTube URL, search term, or Spotify link.")
    ...
    async def play(self, interaction: discord.Interaction, query: str):
        ...
        if ("youtube.com/playlist" in query or "list=" in query) and "spotify" not in query:
            ...
            await interaction.followup.send(f"Added **{len(songs)}** songs from playlist to queue!")
            return
        song = await self.fetch_song(query, interaction.user)
        if not song:
            await interaction.followup.send("Could not find that song. Try a different search term or URL.")
            return
    ...
    async def queue_cmd(self, interaction: discord.Interaction):
        ...
        if state.current:
            embed.add_field(name="Now Playing", value=f"[{state.current.title}]({state.current.webpage_url}) ({state.current.duration_str()})", inline=False)
        if state.queue:
            lines = "\n".join(f"`{i+1}.` [{s.title}]({s.webpage_url}) — {s.duration_str()}" for i, s in enumerate(list(state.queue)[:10]))
            embed.add_field(name=f"Up Next ({len(state.queue)} songs)", value=lines, inline=False)
        else:
